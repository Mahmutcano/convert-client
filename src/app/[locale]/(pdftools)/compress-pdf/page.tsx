...
import { formatFileSize } from '@/utils/getFileExtension';
...
export default function Compress() {
  ...
  const [compressedFileIcon, setCompressedFileIcon] = useState<string | null>(null);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.size > fileSizeLimit) {
        setError(t('FileSizeMustBeLessThan50MB'));
        setFile(null);
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/convert/compress-pdf', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });

      const { fileId, originalSize, compressedSize, compressionRatio } = response.data;

      if (compressionRatio < 0) {
        setError(t('CompressionNotPossible'));
        setLoading(false);
        return;
      }

      setFileId(fileId);
      setCompressedFileIcon('/assets/icons/files/png/016-pdf.png'); // Assuming compressed file is always PDF
        ...
        <CardContent className="p-6 space-y-4">
          {!fileId && (
            <>
              ...
              >
                <Image src="/assets/icons/files/png/016-pdf.png" alt='pdf' width={32} height={32} />
        ...
        <CardFooter>
          {file && !fileId ? (
            <Button size="lg" onClick={handleSubmit} className="flex justify-center items-center space-x-2 w-full">
              <ArrowLeftRight className='w-4 h-4' /> <p>{t('Compress')}</p>
            </Button>
          ) : (
            <Button size="lg" onClick={handleDownload} className="flex justify-center items-center space-x-2 w-full">
              <Download className='w-4 h-4' /> <p>{t('DownloadCompressedPDF')}</p>
            </Button>
          )}
        </CardFooter>
      </Card>
