...
export default function PdfToPowerpointConversion() {
  ...
  const handleSubmit = async () => {
    ...
    formData.append('outputExt', ".pptx");
    ...
    try {
      ...
      setConvertedFileIcon('/assets/icons/files/png/032-ppt.png'); // Assuming converted file is always PowerPoint
  ...
  const handleDownload = async () => {
    if (fileId) {
      try {
        ...
        link.href = url;
        link.setAttribute('download', file!.name.replace(/\.[^/.]+$/, "") + ".pptx");
    ...
    <div className='flex flex-col justify-center items-center py-24 md:py-24 lg:py-24'>
      <div className='flex flex-col justify-center items-center mb-4 space-y-2'>
        <h1 className="text-3xl font-semibold tracking-tighter sm:text-3xl text-center">
          {t('ConvertPDFToPowerpoint')}
        </h1>
        <h1 className="max-w-[700px] text-muted-foreground md:text-sm/relaxed text-center">
          {t('YouCanTurnPdfFilesIntoEditablePowerpointFilesByConvertingThemToPowerpoint')}
        </h1>
      </div>
          ...
          {convertedFileIcon && fileId && (
            <div className='flex flex-col justify-center items-center'>
              ...
              <div className="mt-4 text-xs text-muted-foreground text-center">
                {file!.name.replace(/\.[^/.]+$/, ".pptx")} <br />({formatFileSize(file!.size)})
              </div>
            </div>
          )}
        </CardContent>
