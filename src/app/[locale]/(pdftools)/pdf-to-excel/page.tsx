...
export default function PdfToExcelConversion() {
  ...
  const handleSubmit = async () => {
    ...
    formData.append('outputExt', ".xlsx");
    ...
    try {
      ...
      setConvertedFileIcon('/assets/icons/files/png/031-xls.png'); // Assuming converted file is always Excel
  ...
  const handleDownload = async () => {
    if (fileId) {
      try {
        ...
        link.href = url;
        link.setAttribute('download', file!.name.replace(/\.[^/.]+$/, "") + ".xlsx");
    ...
    <div className='flex flex-col justify-center items-center py-24 md:py-24 lg:py-24'>
      <div className='flex flex-col justify-center items-center mb-4 space-y-2'>
        ...
        <h1 className="max-w-[700px] text-muted-foreground md:text-sm/relaxed text-center">
          {t('YouCanTurnPdfFilesIntoEditableExcelFilesByConvertingThemToExcel')}
        </h1>
      </div>
          ...
          {convertedFileIcon && fileId && (
            <div className='flex flex-col justify-center items-center'>
              ...
              <div className="mt-4 text-xs text-muted-foreground text-center">
                {file!.name.replace(/\.[^/.]+$/, ".xlsx")} <br />({formatFileSize(file!.size)})
              </div>
            </div>
          )}
        </CardContent>
