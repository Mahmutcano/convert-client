...
export default function PdfToWordConversion() {
  ...
  const handleSubmit = async () => {
    ...
    formData.append('outputExt', ".docx");
    ...
    try {
      ...
      setConvertedFileIcon('/assets/icons/files/png/033-doc.png'); // Assuming converted file is always Word
  ...
  const handleDownload = async () => {
    if (fileId) {
      try {
        ...
        link.href = url;
        link.setAttribute('download', file!.name.replace(/\.[^/.]+$/, "") + ".docx");
    ...
    <div className='flex flex-col justify-center items-center py-24 md:py-24 lg:py-24'>
      <div className='flex flex-col justify-center items-center mb-4 space-y-2'>
        ...
        <h1 className="max-w-[700px] text-muted-foreground md:text-sm/relaxed text-center">
          {t('YouCanTurnPdfFilesIntoEditableWordFilesByConvertingThemToWord')}
        </h1>
      </div>
          ...
          {convertedFileIcon && fileId && (
            <div className='flex flex-col justify-center items-center'>
              ...
              <div className="mt-4 text-xs text-muted-foreground text-center">
                {file!.name.replace(/\.[^/.]+$/, ".docx")} <br />({formatFileSize(file!.size)})
              </div>
            </div>
          )}
        </CardContent>
